...
from PyQt5.QtSql import *
class moveinfo(QDialog, Ui_Dialog):
    def __init__(self, username):
        ...
        self.db.setConnectOptions('QSQLITE_OPEN_URI;QSQLITE_OPEN_READWRITE;QSQLITE_OPEN_EXCLUSIVE')
        self.connect_db()
    ...
    def On_Login(self):
        ...
        if (self.lineEdit.text() and self.lineEdit_2.text() and self.lineEdit_3.text() and self.lineEdit_4.text() and self.lineEdit_5.text() and self.lineEdit_6.text()):
            query.prepare("UPDATE userinfo SET 出生日期=?, 籍贯=?, 学历=?, 性别=?, 实习开始时间=?, 实习结束时间=?, 所属区段=? WHERE 姓名=?")
